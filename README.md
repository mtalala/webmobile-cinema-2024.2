...
- Orientação Landscape: Divisão em três colunas para melhor aproveitamento do espaço horizontal.
  ...
  Os arquivos ```globals.css``` e ```page.module.css``` trabalham em conjunto para criar uma interface funcional, responsiva e agradável visualmente.

O ```globals.css``` define estilos globais, assegurando a uniformidade no design e suportando temas claro e escuro. Já o ```page.module.css``` fornece a estilização específica da página, com foco na disposição dos elementos e na interação com o usuário. Essa divisão de responsabilidades contribui para a organização do código, tornando mais fácil a manutenção e a escalabilidade do projeto.

Um design bem estruturado, elementos interativos e adaptação a diferentes tamanhos de tela tornam essa aplicação acessível e moderna, garantindo uma experiência satisfatória ao usuário.
